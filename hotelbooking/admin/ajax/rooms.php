<?php 
    ...
    if(isset($_POST['add_room'])){
        ...
        if($stmt = mysqli_prepare($con, $q2)){
            ...
            $flag = 0;
            die('query canot be prepard - insert');
        }

        $q3 = "INSERT INTO `room_features`(`room_id`, `features_id`) VALUES (?,?)";
        // Lặp qua các đặc điểm và ràng buộc tham số cho mỗi đặc điểm
        if($stmt = mysqli_prepare($con, $q3)){
            ...
            $flag = 0;
            die('Query cannot prepare - insert');
        }
    ...
    if (isset($_POST['edit_room'])) 
    {
        ...
        if ($stmt = mysqli_prepare($con, $q2)) {
            ...
            $flag = 0;
            // Log or handle the error appropriately instead of using die
...
if (isset($_POST['get_room_images'])) {
    ...
    $res = select($q, [$frm_data['get_room_images']], 'i');
    $path = ROOMS_IMG_PATH;

    while ($row = mysqli_fetch_assoc($res)) {
        $thumb_btn = ($row['thumb'] == 1) ? "<i class='bi bi-check-lg text-light bg-success px-2 py-1 rounded fs-5'></i>" : "
            <button onclick='thumb_image($row[sr_no], $row[room_id])' class='btn btn-secondary btn-sm shadow-none'>
